...
void PutCharMag(unsigned short Xpos,unsigned short Ypos,unsigned char c,unsigned short charColor,unsigned short bkColor, float magnifier)
{
  unsigned short i=0;
  unsigned short j=0;
  unsigned char buffer[16];
  ...
  for (i=0;i<16;i++)
  {
    ...
    for (j=0;j<8;j++)
    {
      if ( (tmp_char >> 7-j) & 0x01 == 0x01)
        {
          LCD_SetPoint(Xpos+j*magnifier,Ypos+i*magnifier,charColor);  /* 字符颜色 */
        }
        else
        {
          LCD_SetPoint(Xpos+j*magnifier,Ypos+i*magnifier,bkColor);  /* 背景颜色 */
        }
    }
  }
}
...
uint8_t TempChar;
 do
  {
    TempChar=*str++;  
    PutCharMag(Xpos,Ypos,TempChar,Color,bkColor,magnifier);    
    if (Xpos<232)
    {
      Xpos+=8*magnifier;
    } 
