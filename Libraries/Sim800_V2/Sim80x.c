...
//######################################################################################################################
void	Sim80x_RxCallBack(char recievedChar)
{
  ...
  else if(Sim80x.Status.DataTransferMode==1)
  {
	...
	Sim80x.UsartRxTemp = recievedChar;
//	Sim80x.UsartRxTemp = USART_ReceiveData(GSM_UART);
//	HAL_UART_Receive_IT(&_SIM80X_USART,&Sim80x.UsartRxTemp,1);	
}
//######################################################################################################################
...
void StartSim80xTask(void )
{ 
  static uint32_t TimeForSlowRun=0;
  #if( _SIM80X_USE_GPRS==1)
  static uint32_t TimeForSlowRunGPRS=0;
  #endif
  static uint8_t UnreadMsgCounter=1;
//  while(1)
 // {    
    //###########################################
    #if( _SIM80X_USE_BLUETOOTH==1)
