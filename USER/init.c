...
GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;					    //USER KEY A
  ...
  GPIO_Init(GPIOB, &GPIO_InitStructure);*/
	...
	//PIN_CONFIGURATION(GP_LED);
	PIN_CONFIGURATION(USR_BTN);
	PIN_CONFIGURATION(ZE_DAC);
	PIN_CONFIGURATION(BQ_nEN);
	PIN_CONFIGURATION(BQ_nCHG);
	PIN_CONFIGURATION(BQ_ISET2); PIN_ON(BQ_ISET2);
	PIN_CONFIGURATION(BAT_ADC);
	PIN_CONFIGURATION(DISC_USB);
	PIN_CONFIGURATION(DS18b20_Q);
	PIN_CONFIGURATION(GP_OUT);
	PIN_CONFIGURATION(GAS_AO);
	PIN_CONFIGURATION(SCL_PIN);
	PIN_CONFIGURATION(SDA_PIN);
	PIN_CONFIGURATION(DHT_DATA);
